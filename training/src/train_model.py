...
def load_data(path: DictConfig):
    ...
    Args:
        path (DictConfig): Configuration object containing paths to data files.

    Returns:
        tuple: Tuple containing X_train, X_test, y_train, y_test DataFrames.
    """
...
def get_objective(
    ...
    Args:
        ...
        space (dict): Search space for hyperparameter optimization.

    Returns:
        dict: Dictionary containing the loss, status, and trained model.
    """
    model = XGBClassifier(
...
def optimize(objective: Callable, space: dict):
    ...
    Args:
        objective (Callable): Objective function for optimization.
        space (dict): Search space for hyperparameter optimization.
